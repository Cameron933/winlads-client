...
const FutureRaffles = ({ setShowPopup, showLessPopUP }) => {
  ...
  return raffleArray3.map((round, key) => (
    <div
      key={key}
      ...
      style={{
        background: "linear-gradient(98.92deg, #EF4444 45%, #000000 83%)",
      }}
    ...
    >
      ...
      <div className="flex text-end flex-col z-10">
        <p className="text-white font-bold xl:text-sm text-xs">{round.name}</p>
      ...
      <div className="flex justify-between px-5 items-center w-full">
        <div className="w-10/12 overflow-clip flex gap-2 z-10">
          ...
          <p className="text-white text-center font-extrabold w-full whitespace-nowrap flex justify-center gap-6">
            {" "}
